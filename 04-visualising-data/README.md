...
|------------------------|----------|
| Change over time | Line chart or column chart |
| Comparison between categories | Bar chart (horizontal) or column chart (vertical) |
| Part of a whole | Stacked bar chart or simple callout (avoid pie charts with more than 3 segments) |
| Correlation or relationship | Scatter plot |
| A single important number | Large text callout, not a chart |
| Progress toward a target | Bullet chart or simple bar with target line |
...
**The pie chart with 8 segments.** Human eyes cannot accurately compare pie slices, especially when there are more than three. Use a horizontal bar chart instead — the differences are immediately readable.

**The 3D chart.** Three-dimensional effects make it harder to read actual values. Use flat charts. Always.
...
Use conditional formatting, bold text, or a colour highlight to draw the eye to the point you are making. The audience should not have to search for it.

### Example

| Before | After |
|--------|-------|
| Title: "Project Status Q3" — full table, 12 rows, 8 columns, no formatting | Title: "Two projects need a decision before Q4 begins" — 3 rows, 4 columns, risk rows highlighted in amber |
...
**Burying the insight in the body text.** If the most important number is in bullet point 6 of 9, your audience will miss it or arrive at it too late. Lead with the insight.
...
**Letting the data decide the story.** The data should support your argument, not generate it. Know what you want to say first, then find the data that proves it. If the data contradicts your argument, that is useful information — but it is a different conversation.
